...
class CliConfigLoader(
    ...
    fun load(workspaceRoot: Path): CliConfig {
        val globalProperties = readProperties(userHome.resolve(".config").resolve("agent").resolve("config.properties"))
        val workspaceProperties = readProperties(workspaceRoot.resolve(".agent").resolve("config.properties"))
